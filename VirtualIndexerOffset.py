...
class VirtualIndexer:
    def __init__(self, maxLength=1, offset = 0, return_per_trigger = False):
    ...
    def exportData(self,path):
        ...
        with open(path, 'wb') as handle:
            if not self.offset == 0:
                pickle.dump(self.getOffsetDict(), handle, protocol=pickle.HIGHEST_PROTOCOL)
            else:        
    ...
    def getCurrentIndex(self):
        if not self.offset == 0:
            return self.currentIndex + self.offset
        else:
    ...
    def getOffsetDict(self):
        if not self.offset == 0:
            ...
            ## Make copy of masterDict
            buffer_dict = self.masterDict
            ## Save state variable and pop it
            state = buffer_dict.get('State')

            ## Only offset if state 2 and state 3
            if state > 1:
                ## Get all indices
                indices = list(buffer_dict.keys())
                num_of_indices = len(indices) - 1             ## Dont Count State key 
                ## Offset all indexes
                offset_dict = {}
                for k,v in buffer_dict.items():
                    if not (k == 'State'):
                        newKey = int(k)+self.offset
                        if newKey >= num_of_indices:
                            newKey = newKey - num_of_indices
                        elif newKey < 0 and state == 3:
                            newKey = num_of_indices + newKey 
                        offset_dict[str(newKey)] = v 
                ## Restore state key
                offset_dict['State'] = state
